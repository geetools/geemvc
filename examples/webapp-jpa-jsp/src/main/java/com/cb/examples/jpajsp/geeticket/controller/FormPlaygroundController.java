...
public class FormPlaygroundController {
    ...
    @Request("/playground")
    public Result viewTicketDetails() {
        ...
        return Results.view("playground/form_playground").bind("strVal", "two").bind("intVal", 2).bind("ticket", tickets.get(0)).bind("selectedTypes", selectedTypes).bind("allTags", allTags).bind("testBean", new TestBean("two", 2, 1, 3, 5, 7, 9))
    ...
    public Result saveForm(@PathParam("id") Long id, Ticket ticket, TestBean testBean) {
        return Results.redirect("/forms/playground");
    }

    @Request(path = "/save/{id:[\\d]+}", name = "unique-name-2", method = HttpMethod.PUT)
    public Result saveForm(@PathParam("id") Long id, Ticket ticket) {
        return Results.redirect("/forms/playground");
    }

    @Request(path = "/save", name = "unique-name-3")
    public Result saveForm(Ticket ticket) {
        return Results.redirect("/forms/playground");
    }

    @Request(path = "/save", name = "unique-name-4")
    public Result saveForm3(Ticket ticket) {
        return Results.redirect("/forms/playground");
    }
}
